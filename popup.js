...
document.addEventListener('DOMContentLoaded', () => {
  ...
  chrome.storage.local.get([
    'github_token', 'github_repo_owner', 'github_repo_name', 'stats'
  ], (data) => {
    ...
    // Clear any error badge when the user opens the popup
  ...
  function showStatus(message, type) {
    ...
    // Reset all classes and any inline styles before applying the new type class
    statusMsg.className = 'status-message';
    statusMsg.style.color = '';

    if (type === 'success') statusMsg.classList.add('status-success');
    else if (type === 'error') statusMsg.classList.add('status-error');
    else statusMsg.classList.add('status-info');
  }
  ...
  function renderStats(stats) {
    const currentStats = stats || { Easy: 0, Medium: 0, Hard: 0, total: 0, history: [] };
